...
import { mockPlanItems, mockSuggestions, mockSurveys } from '../lib/mockData'
...
const CHART_COLORS = ['#e5e7eb', '#3b82f6', '#eab308', '#22c55e']
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      if (isDemoMode()) {
        setItems(mockPlanItems)
        setSuggestionCount(mockSuggestions.length)
        setNewSuggestions(mockSuggestions.filter(s => s.status === 'new').length)
        setSurveyCount(mockSurveys.length)
        setLoading(false)
        return
      }
      const [{ data: planData }, { data: suggestions }, { data: surveys }] = await Promise.all([
        supabase.from('plan_items').select('*').eq('year', 2026),
        supabase.from('suggestions').select('id, status'),
        supabase.from('surveys').select('id'),
      ])
      setItems((planData ?? []) as PlanItem[])
      const sugs = (suggestions ?? []) as { id: string; status: string }[]
      setSuggestionCount(sugs.length)
      setNewSuggestions(sugs.filter(s => s.status === 'new').length)
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="mb-2">
        <h1 className="text-2xl font-bold text-gray-900">Dashboard B Corp 2026</h1>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">{done} de {total} ítems</p>
            </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">de {total} ítems B Corp</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">{suggestionCount}</p>
              {newSuggestions > 0 && (
                <p className="text-xs text-red-500 mt-1">{newSuggestions} sin leer</p>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-1">{surveyCount}</p>
              <p className="text-xs text-gray-400 mt-1">lanzadas este año</p>
            </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 col-span-2">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Progreso por categoría</h2>
          <div className="space-y-3">
            {categoryCounts.map(({ category, fullCategory, total, done }) => (
              <div key={category}>
                <div className="flex items-center justify-between mb-1">
                  <Badge label={fullCategory} className={`text-xs ${CATEGORY_COLORS[fullCategory] ?? 'bg-gray-100 text-gray-700'}`} />
      ...
      <div className="grid grid-cols-2 gap-6">
        {/* Responsible chart */}
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Carga por responsable</h2>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={responsibleCounts} layout="vertical" margin={{ left: 0, right: 16 }}>
              ...
              <Tooltip />
              <Bar dataKey="total" fill="#e5e7eb" radius={[0, 4, 4, 0]} name="Total" stackId="a" />
        ...
        {/* Upcoming deadlines */}
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Próximas fechas límite</h2>
          {upcoming.length === 0 ? (
              ...
              {upcoming.map(item => {
                ...
                return (
                  <div key={item.id} className="flex items-start gap-3">
                    {isUrgent && <AlertCircle size={14} className="text-red-500 mt-0.5 flex-shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-900 truncate">{item.document_name}</p>
