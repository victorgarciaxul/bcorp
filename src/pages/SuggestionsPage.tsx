            ...
            <button key={item.id} onClick={() => setSelected(item)}
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-xs text-gray-500">{format(new Date(item.created_at), 'dd MMM yyyy', { locale: es })}</p>
                  {item.author_name && <p className="text-xs text-gray-500 mt-0.5">{item.author_name}</p>}
                </div>
              </div>
            </button>
          ))}
